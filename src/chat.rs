...
async fn main() -> anyhow::Result<()> {
    ...
    let system_message = ChatCompletionRequestMessage::System(
        ChatCompletionSystemMessage::new("You're function call AI", None),
    );
    chat_request.messages.push(system_message);
...
================================== Running in interactive mode. ===================================\n
    ...
    loop {
        ...
        let escaped_json_str = to_string_pretty(&chat_request).expect("Failed to pretty print JSON");
        let unescaped_json_str = escaped_json_str.replace("\\\"", "\"");
